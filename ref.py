...
class CUB_DG_Dataset(torch.utils.data.Dataset):
    def __init__(self, domain, gvrt_flag, class_caption_flag):
        ...
        self.class_caption_flag = class_caption_flag
        if self.gvrt_flag:
    ...
    def __getitem__(self, img_id):
        ...
        if self.gvrt_flag:
            if not self.class_caption_flag:
                img_id_t = img_id
                file_name_t = file_name
                rand_idx = np.random.randint(len(self.coco.imgToAnns[img_id_t]))
            else:
                img_id_t = list(set(self.coco.catToImgs[image_dict["label"]]))[0]
                file_name_t = self.coco.loadImgs(img_id_t)[0]["file_name"]
                rand_idx = 0
...
def get_datasets_and_iterators(test_env, gvrt_flag, class_caption_flag, single_domain_flag, eval_flag=False):
    ...
    for d, domain in enumerate(DOMAINS):
        dataset = CUB_DG_Dataset(domain, gvrt_flag, class_caption_flag)
        datasets.append(dataset)

        if not single_domain_flag:
            ...
            if not eval_flag and d == test_env:
                iterators_train.append(InfiniteDataLoader(
                    _SplitDataset(dataset, dataset.split["train"], train_flag=True), BATCH_SIZE, NUM_WORKERS))
                iterators_eval.append(torch.utils.data.DataLoader(
                    _SplitDataset(dataset, dataset.split["valid"], train_flag=False),
                    batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=False, pin_memory=True))
                names_eval.append("env%d_1" % d)
                source_num += 1
            if d != test_env:
